...
if (length(file_arg) != 1L) {
  fail("Unable to locate build_backgammoncalculator_docs.R")
}
...
destination <- file.path(site_output, "about", "backgammoncalculator")
...
package_repo <- Sys.getenv(
  "BACKGAMMONCALCULATOR_REPOSITORY",
  "https://github.com/backgammonsimplified/backgammoncalculator.git"
)
package_ref <- Sys.getenv("BACKGAMMONCALCULATOR_REF", "master")
package_source <- Sys.getenv("BACKGAMMONCALCULATOR_SOURCE", "")

workspace <- tempfile("backgammoncalculator-docs-")
...
if (nzchar(package_source)) {
  ...
  package_path <- file.path(workspace, "backgammoncalculator")
  ...
  if (!all(copied)) {
    fail("Unable to copy local backgammoncalculator source into build workspace")
  }

  message("Building backgammoncalculator docs from local source: ", source_path)
} else {
  package_path <- file.path(workspace, "backgammoncalculator")
  message("Fetching backgammoncalculator ref ", package_ref)
  run(
    "git",
    c(
      "clone",
...
message("backgammoncalculator documentation: ", destination)
